...
def plot_3d_motion(save_path, kinematic_tree, joints, title, text='', figsize=(10, 10), fps=120, radius=4, color_range=None):
#     matplotlib.use('Agg')
    ...
    if len(title_sp) > divider:
        ...
        title = '\n'.join([' '.join(title_sp[i*divider:(i+1)*divider]) for i in range(chunks)])
    
    text_sp = text.split(' ')
    if len(text_sp) > divider:
        chunks = np.ceil(len(text_sp) / divider).astype(int)
        text = '\n'.join([' '.join(text_sp[i*divider:(i+1)*divider]) for i in range(chunks)])
        ...
        for i, (chain, color) in enumerate(zip(kinematic_tree, colors)):
            ...
            if i < 5:
                ...
                for i, point in enumerate(chain):
                    psize = 50.0
                    ax.scatter(data[index, point, 0], data[index, point, 1], data[index, point, 2], color="#382be3", s=psize)
            else:
                linewidth = 2.0 #*10
                color = [254/255, 78/255, 126/255, 0.9]
            ax.plot3D(data[index, chain, 0], data[index, chain, 1], data[index, chain, 2], linewidth=linewidth, color=color, zorder=1.1)
            # print(index, chain)
            
                # print(trajec[:index, 0].shape)
        
        
        plot_xzPlane(MINS[0]-trajec[0, 0] - 3, MAXS[0]-trajec[0, 0] + 3, 0, MINS[2]-trajec[0, 1] , MAXS[2]-trajec[0, 1]+ 3)
...
if __name__ == "__main__":
    ...
    for reaction_set in set_paths:
        ...
        # 
        # plot pred
        plot_3d_motion(save_path_pred, beat_kinematic_chain, lsn_pred, title="", text='WEG on:' + ",".join(focus_words), fps=25, radius=4)
        # plot_3d_motion(save_path_pred.replace('pred', 'pred2'), beat_kinematic_chain, lsn_pred, title="", text='', fps=25, radius=4)
        
        # # plot gt
        plot_3d_motion(save_path_gt, beat_kinematic_chain, lsn_gt, title="", text="", fps=25, radius=4)

        # # plot sem
        if os.path.exists(sem_path):
            plot_3d_motion(save_path_sem, beat_kinematic_chain, lsn_semantic, title="", text='No WEG off:' + ",".join(focus_words), fps=25, radius=4)
        
        
        # # ffmpeg add audio to video
        subprocess.run(shlex.split(f"ffmpeg -i {save_path_pred} -i {pjoin(reaction_set, 'lsn_audio.wav')} -map 0:v -map 1:a -c:v copy -acodec mp3 {save_path_pred_audio} -y -loglevel error"))
        subprocess.run(shlex.split(f"ffmpeg -i {save_path_gt} -i {pjoin(reaction_set, 'lsn_audio.wav')} -map 0:v -map 1:a -c:v copy -acodec mp3 {save_path_gt_audio} -y -loglevel error"))
        # 
        if os.path.exists(sem_path):
            subprocess.run(shlex.split(f"ffmpeg -i {save_path_sem} -i {pjoin(reaction_set, 'lsn_audio.wav')} -map 0:v -map 1:a -c:v copy -acodec mp3 {save_path_sem_audio} -y -loglevel error"))
        
        if os.path.exists(sem_path):
            save_path_combined = pjoin(reaction_set, 'combined.mp4')
            save_path_combined_audio = pjoin(reaction_set, 'combined_audio.mp4')
            subprocess.run(shlex.split(f"ffmpeg -i {save_path_gt_audio} -i {save_path_pred_audio} -i {save_path_sem_audio} -filter_complex \"[0:v][1:v][2:v]hstack=inputs=3[v]\" -map \"[v]\"  -acodec mp3 {save_path_combined} -y -loglevel error"))
            subprocess.run(shlex.split(f"ffmpeg -i {save_path_combined} -i {pjoin(reaction_set, 'lsn_audio.wav')} -map 0:v -map 1:a -c:v copy -acodec mp3 {save_path_combined_audio} -y -loglevel error"))
