...
class BASEDataModule(pl.LightningDataModule):
    ...
    def get_sample_set(self, overrides={}):
        ...
        if self.cfg.TRAIN.STAGE == "vae":
            self.split_root = eval(f"self.cfg.DATASET.{self.name.upper()}.VAE_SPLIT_ROOT")
            split_files = [pjoin(
                self.split_root[i],
                self.cfg.EVAL.SPLIT + ".txt") for i in range(len(self.split_root))]
            split_file = split_files[0] if len(split_files) == 1 else split_files
        elif self.cfg.TRAIN.STAGE == "diffusion":
            # breakpoint()
            self.split_root = eval(f"self.cfg.DATASET.{self.name.upper()}.SPLIT_ROOT")
            split_files = [pjoin(
                self.split_root[i],
                self.cfg.EVAL.SPLIT + ".txt") for i in range(len(self.split_root))]
            split_file = split_files[0] if len(split_files) == 1 else split_files
        ...
        if item.endswith("_dataset") and not item.startswith("_"):
            ...
            if item_c not in self.__dict__:
                ...
                if self.cfg.TRAIN.STAGE == "vae":
                    ...
                    split_files = [pjoin(
                        self.split_root[i],
                        eval(f"self.cfg.{subset}.SPLIT") + ".txt") for i in range(len(self.split_root))]
                    split_file = split_files[0] if len(split_files) == 1 else split_files
                elif self.cfg.TRAIN.STAGE == "diffusion":
                    # split_file = pjoin(
                    # eval(f"self.cfg.DATASET.{self.name.upper()}.REACT_SPLIT_ROOT"),
