# Taken from BEAT dataset/CaMN repo:
import random
import torch
...
class PoseEncoderConv(nn.Module):
    def __init__(self, length, dim, feature_length=32):
        ...
        self.net = nn.Sequential(
            ConvNormRelu(dim, self.base, batchnorm=True), # 126
            ConvNormRelu(self.base, self.base*2, batchnorm=True), # 124
            ConvNormRelu(self.base*2, self.base*2, True, batchnorm=True), # 61
            nn.Conv1d(self.base*2, self.base, 3) # 59
        )
        self.out_net = nn.Sequential(
            nn.Linear(59*self.base, self.base*20),  # for 128 frames
            nn.BatchNorm1d(self.base*20),
            nn.LeakyReLU(True),
            nn.Linear(self.base*20, self.base*4),
...
class PoseDecoderConv(nn.Module):
    def __init__(self, length, dim, use_pre_poses=False, feature_length=32):
        ...
        if length == 64:
            ...
            self.pre_net = nn.Sequential(
                nn.Linear(self.feat_size, self.feat_size*3),
                nn.BatchNorm1d(self.feat_size*3),
                nn.LeakyReLU(True),
                nn.Linear(self.feat_size*3, self.feat_size*7),
                nn.BatchNorm1d(self.feat_size*7),
                nn.LeakyReLU(True),
                nn.Linear(self.feat_size*7, self.feat_size//8*128),
            )
        else:
            assert False
...
class HalfEmbeddingNet(nn.Module):
    def __init__(self, pose_length, pose_dim, feature_length=300):
        super().__init__()
        n_frames = pose_length
        pose_dim = pose_dim
